...
async function safeFetch(url, options = {}) {
  try {
    ...
    console.error("Network error (MostPopular):", err);
...
const MostPopular = () => {
  ...
  const loadMostPopularAnimes = async (page = 1) => {
    if (loading) return;
    setLoading(true);

    const result = await safeFetch(`http://localhost:3030/api/v1/animes/most-popular?page=${page}`);

    if (result.ok && result.data?.success) {
      setAnimes(result.data.data?.response || []);
      setHasNextPage(!!result.data.data?.pageInfo?.hasNextPage);
    } else if (!result.networkError) {
      console.error("❌ Error loading most popular animes:", result);
    }
  ...
  useEffect(() => {
    loadMostPopularAnimes(currentPage);
  }, [currentPage]);
